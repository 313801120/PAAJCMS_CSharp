<%
/************************************************************
×÷Õß£ººì³¾ÔÆËï(SXY) ¡¾¾«Í¨ASP/PHP/ASP.NET/VB/JS/Android/Flash£¬½»Á÷ºÏ×÷¿ÉÁªÏµ±¾ÈË)
°æÈ¨£ºÔ´´úÂëÃâ·Ñ¹«¿ª£¬¸÷ÖÖÓÃÍ¾¾ù¿ÉÊ¹ÓÃ¡£ 
´´½¨£º2016-08-04
ÁªÏµ£ºQQ313801120  ½»Á÷Èº35915100(ÈºÀïÒÑÓÐ¼¸°ÙÈË)    ÓÊÏä313801120@qq.com   ¸öÈËÖ÷Ò³ sharembweb.com
¸ü¶à°ïÖú£¬ÎÄµµ£¬¸üÐÂ¡¡Çë¼ÓÈº(35915100)»òä¯ÀÀ(sharembweb.com)»ñµÃ
*                                    Powered by PAAJCMS 
************************************************************/
%>
...
void testGetPY(){
    string c=""; string[] splStr; int i=0; string s=""; string cn=""; string en="";
...
string pinYin(string content, string sType){
    string[] py=aspArray(402); string[] splStr; string en=""; string s=""; string c=""; int i=0; int j=0; string c2=""; string C3=""; string C4=""; string En2=""; string En3=""; string En4="" ;int nLen=0;
    ...
    if( sType== "Æ´Òô" ){
        for( i= 1 ; i<= len(content); i++){
            s= mid(content, i, 1);
            if( inStr("0123456789_abcdefghijklmnopqrstuvwxyz", s)== 0 ){
                for( j= 0 ; j<= uBound(py); j++){
                    if( inStr(py[j], s) > 0 ){
                        nLen= inStrRev(py[j], "_") + 1;
                        en= mid(py[j],nLen,-1);
    ...
    if( inStr("|ºº×Ö|ºº×Ö´òÓ¡|", "|" + sType + "|") > 0 ){
        ...
        if( sType== "ºº×Ö´òÓ¡" ){ echo("Æ´Òô×ªºº×Ö", c) ;}
        pinYin= c;
    }else{
        for( i= 1 ; i<= len(content); i++){
        ...
        if( sType== "1" ){
            ...
            echo("×ª»»×Ö·û", content);
            echo("Æ´ÒôÐ¡Ð´", aspTrim(c));
            echo("Ê××ÖÄ¸´óÐ´", aspTrim(C4));
            echo("È¡Ê××ÖÄ¸", aspTrim(c2));
            echo("¹«Ë¾±ê×¼", enToCompany(C4));
            echo("ÎÞ¿Õ¸ñÊ××ÖÄ¸Ð¡Ð´", aspTrim(replace(c, " ", "")));
            echo("ÎÞ¿Õ¸ñÊ××ÖÄ¸´óÐ´", aspTrim(replace(C3, " ", "")));
        }
    }
    return pinYin;
}
...
//Æ´Òô
string pinYin2(string content){
    return pinYin(content, "Æ´Òô");
}


//´¦Àí¹«Ë¾Ó¢ÎÄµØÖ·
string enToCompany( string content){
    content= replace(content, "Shang Hai", "Shanghai");
    content= replace(content, "You Xian Gong Si", " Co.,Ltd.");
    return aspTrim(content);
}
...
string simplifiedChinese(string content){
    return handleTransferChinese(content,1);
}
//¼òÌå×ª·±Ìå Simplified transfer
string simplifiedTransfer(string content){
    return handleTransferChinese(content,0);
}
//¼òÌå ·±Ìå ×ª»»    0Îª¼òÌå×ª·±Ìå  1Îª·±Ìå×ª¼òÌå
string handleTransferChinese(string content,int nType){
    string zd="";string s="";string[] splstr;string[] splxx;
...
string handleTransferChinese_temp(string content,int nType){
